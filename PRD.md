...
- 포토샵 수준의 RAW 편집, 진짜 인쇄소 ICC 프로파일 색 관리 (카메라·인쇄소별 전용 엔진이 필요해 브라우저 앱으로는 불가능하다). 대신 현실적인 근사 버전은 넣었다: 사진 불러오기 전 노출·화이트밸런스·레벨·톤 보정(`src/import/photoAdjust.ts`), 표준 수식 기반 CMYK 근사 미리보기·PDF 내보내기(`src/export/cmyk.ts`) — 정확한 인쇄 색이 아니라 "대략 이렇게 보일 것"이라는 참고용이다.
